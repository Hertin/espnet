...
from espnet.utils.dataset import TransformDatasetEar
...
def clear_lang_losses(trainer):
    trainer.updater.clear_lang_losses()
...
class CustomEvaluator(BaseEvaluator):
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for batch in it:
                langs, x = batch[0], batch[1:]
                x = _recursive_to(x, self.device)
...
class CustomUpdater(StandardUpdater):
    ...
    def __init__(
        ...
        self.ita = ita
        self.ctc_ita = ctc_ita
        self.att_ita = att_ita
        logging.warning(f"CustomUpdater [ita] {self.ita} {self.ctc_ita} {self.att_ita}")
        self.dataset = train_iter["main"].kwargs["dataset"]
        self.num_langs = len(self.dataset.all_lang)
        self.lang_ctc_losses = [AverageMeter() for _ in range(self.num_langs)]
        self.lang_att_losses = [AverageMeter() for _ in range(self.num_langs)]
        self.lang_losses = [AverageMeter() for _ in range(self.num_langs)]
        # logging.warning(f"CustomUpdater [num_langs] {self.num_langs}")

    def clear_lang_losses(self):
        self.lang_ctc_losses = [AverageMeter() for _ in range(self.num_langs)]
        self.lang_att_losses = [AverageMeter() for _ in range(self.num_langs)]
        self.lang_losses = [AverageMeter() for _ in range(self.num_langs)]
        logging.warning(f"CustomUpdater [clear_lang_losses]")

    def update_loss_meter(self, loss_meter, langs, loss):
        for l in range(self.num_langs):
            valid_l_idx = (langs==l) & (loss.cpu()!=0)
            if torch.any(valid_l_idx):
                lang_loss = loss[valid_l_idx]
                loss_meter[l].update(lang_loss.mean().detach(), len(lang_loss))

    def compute_scale(self, loss_meter, langs):
        lang_loss = [ll.avg for ll in loss_meter]
        lang_loss = torch.FloatTensor(lang_loss)
        lang_rank_by_loss = torch.argsort(lang_loss) # ascending
        scale = torch.FloatTensor([0.0 for _ in range(len(langs))]) # batch size

        for rank, l in enumerate(lang_rank_by_loss):
            # print('rank', rank, 'dialect', d, torch.any(dialect==d))
            if torch.any(langs==l):
                scale[langs==l] = rank + 1.0
        scale = scale.to(self.device).detach()
        return scale
    ...
    def update_core(self):
        ...
        # logging.warning(f"update_core [lang] {langs}")
        # logging.warning(f"update_core [x] {x[0].size(), x[1].size(), x[2].size()}")
        ...
        if self.ngpu == 0:
            loss, loss_ctc, loss_att = self.model(*x).mean()
            ...
            loss_ctc /= self.accum_grad
            loss_att /= self.accum_grad
            # logging.warning(f"CustomUpdater {type(loss)} {loss}")

        # logging.warning(f"CustomUpdater [loss] {loss.size()} {loss}")
        # logging.warning(f"CustomUpdater [ctc loss] {loss_ctc.size()} {loss_ctc}")
        # logging.warning(f"CustomUpdater [att loss] {loss_att.size()} {loss_att}")

        # if self.ita != 0:
        # self.update_loss_meter(self.lang_losses, langs, loss)
        # self.update_loss_meter(self.lang_ctc_losses, langs, loss_ctc)
        # self.update_loss_meter(self.lang_att_losses, langs, loss_att)
        
        # scale = scale.to(self.device).detach()
        scale = self.compute_scale(self.lang_losses, langs)
        scale_ctc = self.compute_scale(self.lang_ctc_losses, langs)
        scale_att = self.compute_scale(self.lang_att_losses, langs)

        # logging.warning(f"CustomUpdater {lang_loss}")
        # logging.warning(f"CustomUpdater [langs] {langs.size()} {langs}")

        # logging.warning(f"CustomUpdater [avg loss] {[ll.avg for ll in self.lang_losses]}")
        # logging.warning(f"CustomUpdater [avg ctc loss] {[ll.avg for ll in self.lang_ctc_losses]}")
        # logging.warning(f"CustomUpdater [avg att loss] {[ll.avg for ll in self.lang_att_losses]}")

        # logging.warning(f"CustomUpdater [scale] {scale.size()} {scale}")
        # logging.warning(f"CustomUpdater [scale_ctc] {scale_ctc.size()} {scale_ctc}")
        # logging.warning(f"CustomUpdater [scale_att] {scale_att.size()} {scale_att}")

        fair_loss = loss * scale
        fair_ctc_loss = loss_ctc * scale_ctc
        fair_att_loss = loss_att * scale_att
        loss = loss + self.ita * fair_loss + self.ctc_ita * fair_ctc_loss + self.att_ita * fair_att_loss

        loss = loss.mean()
        # loss = loss[loss!=0].mean()
        # logging.warning(f"CustomUpdater [ita] {self.ita}")
        # if loss != 0.0:
        if self.use_apex:
        ...
        if math.isnan(grad_norm):
            logging.warning(f"{loss} grad norm is nan. Do not update model.")
        else:
            optimizer.step()
        # else:
        #     logging.warning(f"{loss} no valid loss")
        optimizer.zero_grad()
...
def train(args):
    ...
    train_json = OrderedDict(items)

    # logging.warning(f'ASR [train.json] {type(train_json)}')
    ...
    train_iter = ChainerDataLoader(
        dataset=TransformDatasetEar(train, lambda data: converter([load_tr(data)])),
        batch_size=1,
    ...
    valid_iter = ChainerDataLoader(
        dataset=TransformDatasetEar(valid, lambda data: converter([load_cv(data)])),
        batch_size=1,
        shuffle=False,
        collate_fn=lambda x: x[0],
        num_workers=args.n_iter_processes,
    ...
    updater = CustomUpdater(
        ...
        args.accum_grad,
        use_apex=use_apex, 
        ita=args.ita, ctc_ita=args.ctc_ita, att_ita=args.att_ita
    )
    trainer = training.Trainer(updater, (args.epochs, "epoch"), out=args.outdir)
    
    trainer.extend(clear_lang_losses, trigger=(1, 'epoch'))
