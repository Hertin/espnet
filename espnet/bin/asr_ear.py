...
class CustomUpdater(StandardUpdater):
    ...
    def update_core(self):
        ...
        if self.ngpu == 0:
            ...
            loss = (
                data_parallel(self.model, x, range(self.ngpu)) / self.accum_grad
            )
            # logging.warning(f"CustomUpdater {type(loss)} {loss}")

        # logging.warning(f"CustomUpdater {type(loss)} {loss}")
        loss = loss.view(-1)
        # logging.warning(f"CustomUpdater {type(loss)} {loss.size()}")
        ...
        for l in range(self.num_langs):
            ...
            if torch.any(valid_l_idx):
                ...
                self.lang_losses[l].update(loss[valid_l_idx].mean().detach(), len(lang_loss))

        lang_loss = [ll.avg for ll in self.lang_losses]
        ...
        # logging.warning(f"CustomUpdater {langs.size()} {langs}")
        # logging.warning(f"CustomUpdater {scale.size()} {scale}")

        fair_loss = loss * scale
        loss = loss + self.ita * fair_loss
...
def train(args):
    ...
    updater = CustomUpdater(
        ...
        args.grad_noise,
        args.accum_grad,
        use_apex=use_apex, ita=args.ita,
    )
