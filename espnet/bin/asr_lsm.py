...
class CustomConverter(object):
    ...
    def __init__(
        self, subsampling_factor=1, dtype=torch.float32, 
        sim_mat=None, char_list=None, augment_ratio=0.5, lsm_weight=0.2, 
    ):
        ...
        self.confidence = 1 - lsm_weight
        self.augment_ratio = augment_ratio
        self.char_list = char_list

    def random_replace(self, ys):
        roll = torch.rand(1)
        # logging.warning(f'transcript {roll}{self.augment_ratio} {self.confidence} {ys} ')
        
        if roll < self.augment_ratio:
            sim_mat = self.sim_mat[ys] * self.lsm_weight
            # logging.warning(f'sim_mat {sim_mat.size()} {len(ys)}')
            for i, y in enumerate(ys):
                sim_mat[i, y] += self.confidence
                # logging.warning(f'sim_mat {sim_mat[i].sum()} {sim_mat.size()}')
                new_y = np.random.choice(len(self.char_list), p=sim_mat[i])
                # if new_y != ys[i]:
                #     logging.warning(f'{sim_mat[i,y]} {sim_mat[i,new_y]}, {ys[i]} {y} {new_y}, {self.char_list[y]}, {self.char_list[new_y]}')
                ys[i] = new_y
        # logging.warning(f'new transcript {ys}')
        # raise
        return ys
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        ys_pad = pad_list(
            [
                torch.from_numpy(self.random_replace(
...
def train(args):
    ...
    sim_mat = torch.from_numpy(np.load(args.sim_mat))
    # Setup a converter
    if args.num_encs == 1:
        converter = CustomConverter(
            subsampling_factor=model.subsample[0], dtype=dtype, 
            sim_mat=sim_mat, char_list=args.char_list, 
            augment_ratio=args.augment_ratio, lsm_weight=args.lsm_weight
        )
    else:
