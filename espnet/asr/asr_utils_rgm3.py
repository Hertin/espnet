...
def _adadelta_eps_decay(trainer, eps_decay):
    for update_flag in ['1f', '2h', '3p']:
        optimizer = trainer.updater.get_optimizer(update_flag)
        # for chainer
...
def _torch_snapshot_object(trainer, target, filename, savefun):
    ...
    snapshot_dict = {
        ...
        "optimizer_f": trainer.updater.get_optimizer("1f").state_dict(),
        "optimizer_h": trainer.updater.get_optimizer("2h").state_dict(),
        "optimizer_phi": trainer.updater.get_optimizer("3p").state_dict(),
    }
...
def torch_resume(snapshot_path, trainer):
    ...
    trainer.updater.get_optimizer("1f").load_state_dict(snapshot_dict["optimizer_f"])
    trainer.updater.get_optimizer("2h").load_state_dict(snapshot_dict["optimizer_h"])
    trainer.updater.get_optimizer("3p").load_state_dict(snapshot_dict["optimizer_p"])
