...
from espnet.asr.asr_utils_rgm3 import adadelta_eps_decay
from espnet.asr.asr_utils_rgm3 import add_results_to_json
from espnet.asr.asr_utils_rgm3 import CompareValueTrigger
from espnet.asr.asr_utils_rgm3 import format_mulenc_args
from espnet.asr.asr_utils_rgm3 import get_model_conf
from espnet.asr.asr_utils_rgm3 import plot_spectrogram
from espnet.asr.asr_utils_rgm3 import restore_snapshot
from espnet.asr.asr_utils_rgm3 import snapshot_object
from espnet.asr.asr_utils_rgm3 import torch_load
from espnet.asr.asr_utils_rgm3 import torch_resume
from espnet.asr.asr_utils_rgm3 import torch_snapshot
            ...
            for batch in it:
                ...
                with reporter_module.report_scope(observation):
                    ...
                    if self.ngpu == 0:
                        ...
                        data_parallel(self.model, (*x, '3p'), range(self.ngpu))
...
class CustomUpdater(StandardUpdater):
    ...
    def __init__(
        ...
        self.forward_counts = {}
        self.forward_counts['1f'] = 0
        self.forward_counts['2h'] = 0
        self.forward_counts['3p'] = 0
    ...
    def loss_bwd(self, update_flag, x):
        ...
        if self.ngpu == 0:
            loss = self.model(*x, update_flag).mean() / self.accum_grad
        else:
            # apex does not support torch.nn.DataParallel
            loss = (
                data_parallel(self.model, (*x, update_flag), range(self.ngpu)).mean() / self.accum_grad
            )
        if self.use_apex:
    ...
    def update_core_K1(self):
        ...
        for update_flag in ['1f','2h', '3p']:
            ...
            # update parameters
            self.forward_counts[update_flag] += 1
            if not is_new_epoch and self.forward_counts[update_flag] != self.accum_grad:
                if update_flag == '3p':
                    return
                else:
                    continue
            self.forward_counts[update_flag] = 0
            # compute the gradient norm to check if it is normal or not
            if update_flag == '1f':

                grad_norm = torch.nn.utils.clip_grad_norm_(
                    list(self.model.encoder_f.parameters()) + list(self.model.ctc_f.parameters()), self.grad_clip_threshold
                )
            elif update_flag == '2h':
                grad_norm = torch.nn.utils.clip_grad_norm_(
                    list(self.model.encoder_h.parameters()) + list(self.model.ctc_h.parameters()), self.grad_clip_threshold
                )
            else:
                grad_norm = torch.nn.utils.clip_grad_norm_(
    ...
    def update_core(self):
        ...
        for k in range(self.num_K):
            for ag in range(self.accum_grad):
                ...
                #     self.loss_bwd('3p')
                if k == save_k:
                    save_list.append(batch)


            grad_norm_1f = torch.nn.utils.clip_grad_norm_(
                list(self.model.encoder_f.parameters()) + list(self.model.ctc_f.parameters()), self.grad_clip_threshold
            )
            grad_norm_2h = torch.nn.utils.clip_grad_norm_(
                list(self.model.encoder_h.parameters()) + list(self.model.ctc_h.parameters()), self.grad_clip_threshold
            )
            logging.info("grad norm 1f ={}".format(grad_norm_1f))
            if math.isnan(grad_norm_1f):
                logging.warning("grad norm 1f is nan. Do not update model.")
            else:
                #pass
                self.get_optimizer('1f').step()
            # print('Zero Grad')
            self.get_optimizer('1f').zero_grad()

            logging.info("grad norm 2h ={}".format(grad_norm_2h))
            if math.isnan(grad_norm_2h):
                logging.warning("grad norm 2h is nan. Do not update model.")
            else:
                #pass
                self.get_optimizer('2h').step()
            # print('Zero Grad')
            self.get_optimizer('2h').zero_grad()
        ...
        for ag in range(self.accum_grad):
            ...
            logging.info("grad norm 3p ={}".format(grad_norm_3p))
            if math.isnan(grad_norm_3p):
                logging.warning("grad norm 3p is nan. Do not update model.")
            else:
                #pass
                self.get_optimizer('3p').step()
            # print('Zero Grad')
            self.get_optimizer('3p').zero_grad()
        self.forward_count = self.num_K
...
def train(args):
    ...
    assert args.num_langs is not None
    ...
    # print(list(model.encoder_phi.parameters()))
    # print(list(model.encoder_f.parameters()))
    # print(list(model.encoder_h.parameters()))
    # Setup an optimizer
    if args.opt == "adadelta":
        optimizer_phi = torch.optim.Adadelta(
            model.encoder_phi.parameters(), rho=0.95, eps=args.eps, weight_decay=args.weight_decay
        )
        optimizer_f = torch.optim.Adadelta(
            list(model.encoder_f.parameters()) + list(model.ctc_f.parameters()), rho=0.95, eps=args.eps, weight_decay=args.weight_decay
        )
        optimizer_h = torch.optim.Adadelta(
            list(model.encoder_h.parameters()) + list(model.ctc_h.parameters()), rho=0.95, eps=args.eps, weight_decay=args.weight_decay
        )
    elif args.opt == "adam":
        optimizer_phi = torch.optim.Adam(model.encoder_phi.parameters(), weight_decay=args.weight_decay)
        optimizer_f = torch.optim.Adam(list(model.encoder_f.parameters()) + list(model.ctc_f.parameters()), weight_decay=args.weight_decay)
        optimizer_h = torch.optim.Adam(list(model.encoder_h.parameters()) + list(model.ctc_h.parameters()), weight_decay=args.weight_decay)
    elif args.opt == "noam":
        ...
        optimizer_phi = get_std_opt(
            model.encoder_phi.parameters(), adim, args.transformer_warmup_steps, args.transformer_lr
        )
        optimizer_f = get_std_opt(
            list(model.encoder_f.parameters()) + list(model.ctc_f.parameters()), adim, args.transformer_warmup_steps, args.transformer_lr
        )
        optimizer_h = get_std_opt(
            list(model.encoder_h.parameters()) + list(model.ctc_h.parameters()), adim, args.transformer_warmup_steps, args.transformer_lr
        )
    else:
    ...
    if args.train_dtype in ("O0", "O1", "O2", "O3"):
        try:
            ...
            raise e
        if args.opt == "noam":
            model, [optimizer_phi.optimizer, optimizer_f.optimizer, optimizer_h.optimizer] = amp.initialize(
                model, [optimizer_phi.optimizer, optimizer_f.optimizer, optimizer_h.optimizer], opt_level=args.train_dtype
            )
        else:
            model, [optimizer_phi, optimizer_f, optimizer_h] = amp.initialize(
                model, [optimizer_phi, optimizer_f, optimizer_h], opt_level=args.train_dtype
            )
        use_apex = True
    ...
    # RGM possible bug here
    setattr(optimizer_phi, "target", reporter)
    setattr(optimizer_phi, "serialize", lambda s: reporter.serialize(s))
    setattr(optimizer_f, "target", reporter)
    setattr(optimizer_f, "serialize", lambda s: reporter.serialize(s))
    setattr(optimizer_h, "target", reporter)
    setattr(optimizer_h, "serialize", lambda s: reporter.serialize(s))
    ...
    updater = CustomUpdater(
        ...
        args.grad_clip,
        {"main": train_iter},
        {'1f':optimizer_f, '2h': optimizer_h, '3p': optimizer_phi},
        device,
        args.ngpu,
        args.num_K,
        args.grad_noise,
    ...
    if args.num_save_attention > 0 and is_attn_plot:
        ...
        att_reporter = plot_class(
            ...
            data,
            args.outdir + "/att_ws",
            converter=lambda x, device: _recursive_to(x[0], device), # dummpy converter
    ...
    if args.opt == "adadelta":
        trainer.extend(
            extensions.observe_value(
                "eps_1f",
                lambda trainer: trainer.updater.get_optimizer("1f").param_groups[0][
                    "eps_1f"
                ],
            ),
            trigger=(args.report_interval_iters, "iteration"),
        )
        report_keys.append("eps_1f")
        trainer.extend(
            extensions.observe_value(
                "eps_2h",
                lambda trainer: trainer.updater.get_optimizer("2h").param_groups[0][
                    "eps_2h"
                ],
            ),
            trigger=(args.report_interval_iters, "iteration"),
        )
        report_keys.append("eps_2h")
        trainer.extend(
            extensions.observe_value(
                "eps_3p",
                lambda trainer: trainer.updater.get_optimizer("3p").param_groups[0][
                    "eps_3p"
                ],
            ),
            trigger=(args.report_interval_iters, "iteration"),
        )
        report_keys.append("eps_3p")
    if args.report_cer:
...
class CustomConverterEval(object):
    """Custom batch converter for Pytorch.
