...
class CustomUpdater(StandardUpdater):
    ...
    def update_core(self):
        ...
        optimizerD = self.get_optimizer('1D')
        optimizerG = self.get_optimizer('2G')
        try:
            # Compute the loss at this time step and accumulate it
            xD = x + tuple(['D'])
            # apex does not support torch.nn.DataParallel
            ce_loss = data_parallel(self.model, xD, range(self.ngpu))
            ce_loss = ce_loss.mean() / self.accum_grad
            self.backward(optimizerD, ce_loss)

            if is_new_epoch or self.forward_count == self.accum_grad:
                grad_norm = torch.nn.utils.clip_grad_norm_(
                    self.model.lang_discriminator.parameters(), self.grad_clip_threshold
                )
            ...
            loss = data_parallel(self.model, xG, range(self.ngpu)).mean() / self.accum_grad

            # update
            
            # update to optimize ctc and and increase classification loss
            self.backward(optimizerG, loss)
...
def train(args):
    ...
    if args.opt == "adadelta":
        ...
        parametersD = list(model.lang_discriminator.parameters())

        parametersG = [p for n, p in model.named_parameters() if 'lang_discriminator' not in n]
        optimizerD = get_std_opt(
            parametersD, args.adim, args.transformer_warmup_steps, args.transformer_lr
        )
        optimizerG = get_std_opt(
            parametersG, args.adim, args.transformer_warmup_steps, args.transformer_lr
        )
    else:
    ...
    if args.train_dtype in ("O0", "O1", "O2", "O3"):
        try:
            ...
            raise e
        if args.opt == "noam":
            model, [optimizerD.optimizer, optimizerG.optimizer] = amp.initialize(
                model, [optimizerD.optimizer, optimizerG.optimizer], opt_level=args.train_dtype
            )
        else:
    ...
    setattr(optimizerD, "target", reporter)
    setattr(optimizerD, "serialize", lambda s: reporter.serialize(s))
    setattr(optimizerG, "target", reporter)
    setattr(optimizerG, "serialize", lambda s: reporter.serialize(s))
