...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        self.embed = Conv2dSubsampling(idim, args.adim, args.dropout_rate)
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        hs_pad, hs_mask = self.embed(xs_pad, src_mask)

        # logging.warning(f'{hs_pad.size()} {hs_mask.size()}')
        # logging.warning(f'{hs_pad[hs_mask.view(batch_size, -1)].size()}')
        
        # hs_pad[hs_mask].mean(dim=1).div(hs_pad[hs_mask], dim=)
        hs_stack = []
        ...
        # logging.warning(f'logit {logit.size()}  {len(truth)} {truth}')
        self.loss = self.criterion(logit, truth)
        ...
        mask = pred == truth
        self.acc = torch.sum(mask) / batch_size
        
        # logging.warning(f'pred {logit} {pred} {pred.size()}')
        # logging.warning(f'acc {self.acc}')
