...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def forward(self, audio_pad, audio_lens, ys_pad):
        ...
        :rtype: float
        """
        # logging.warning(f'{audio_pad.size()} {audio_lens.size()}, {audio_lens}')

        audio_pad_mask = make_pad_mask(audio_lens)
        xs_pad, pad_mask = self.wav2vec.extract_features(audio_pad, audio_pad_mask)

        # logging.warning(f'xs {xs_pad.size()} pad_mask {pad_mask}')
        

        ilens = (~pad_mask).sum(dim=-1)
