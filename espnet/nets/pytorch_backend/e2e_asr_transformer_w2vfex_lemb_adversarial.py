...
class Reporter(chainer.Chain):
    ...
    def report(self, loss_ctc, loss_att, acc, cer_ctc, cer, wer, mtl_loss, lang_loss=None, lang_acc=None):
        ...
        reporter.report({"lang_loss": lang_loss}, self)
        reporter.report({"lang_acc": lang_acc}, self)
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def add_arguments(parser):
        ...
        group.add_argument(
            "--lang-adversarial-ita",
            default=0,
            type=float,
            help="--lang-adversarial-ita",
        )
        group.add_argument(
            "--frame-ratio",
            default=0,
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        self.ita = args.lang_adversarial_ita
        ...
        self.frame_ratio = args.frame_ratio
        self.feature_mapping = nn.Linear(args.wav2vec_idim, args.wav2vec_idim)
        ...
        self.lang_discriminator = nn.Linear(args.wav2vec_idim, len(self.lemb.all_langs))
    ...
    def forward(self, audio_pad, audio_lens, ys_pad, lang_labels, mode=None):
        """E2E forward.
        ...
        # logging.warning(f'pre {features.size()}')
        features = self.feature_mapping(features)
        output_lengths = self._get_feat_extract_output_lengths(audio_lens)
        
        xs, ys = [], []
        for f, ol, ll in zip(features, output_lengths, lang_labels):
            n_frame = int(self.frame_ratio * ol)
            idx = random.sample(list(range(ol)), n_frame)
            
            xs.append(f[idx])
            ys.extend([self.lemb.all_langs.index(ll)] * n_frame)
        xs = torch.cat(xs, dim=0)
        logits = self.lang_discriminator(xs)
        ys = torch.LongTensor(ys).to(xs.device)
        self.ce_loss = self.ce(logits, ys)
        
        preds = torch.argmax(logits, dim=-1)
        self.lang_acc = float(torch.sum(preds == ys)) / len(preds)
        if mode is None:
            assert not self.training
        # logging.warning(f'mode {mode} {self.lang_acc}')
        if mode == 'D':
        ...
        self.loss = self.loss - self.ita * self.ce_loss

        loss_data = float(self.loss)
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
            self.reporter.report(
                loss_ctc_data, loss_att_data, self.acc, cer_ctc, cer, wer, loss_data, 
                lang_acc=self.lang_acc,  lang_loss=float(self.ce_loss.mean())
            )
        else:
