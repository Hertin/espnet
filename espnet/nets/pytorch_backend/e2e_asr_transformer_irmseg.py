...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        self.irm_phone_aware = hasattr(args, 'irm_phone_aware') and args.irm_phone_aware
        self.lang2phid = None
        if self.irm_phone_aware:
            with open(args.irm_lang2ph, 'r') as f:
                self.lang2ph = json.load(f)

            self.lang2phid = {}
            for lang, phones in self.lang2ph.items(): 
                self.lang2phid[lang] = list(map(self.args.char_list.index, phones))
        ...
        if args.mtlalpha > 0.0:
            ...
            self.ctc_irm = CTC(
                ...
                ctc_type=args.ctc_type, reduce=False, ctc_lo=self.ctc.ctc_lo, lang2phid=self.lang2phid
            )
        else:
            self.ctc = None
        ...
        self.rnnlm = None
        self.connected = torch.nn.Linear(args.adim, odim)
        # for name, m in self.named_modules():
        #     logging.warning(f'calculate_all_ctc_probs {name} {name == "ctc"}')
    ...
    def forward(self, xs_pad, ilens, ys_pad, dummy_w=None, lang_labels=None):
        """E2E forward.
        ...
        :rtype: float
        """
        # logging.warning(f'data size {xs_pad.size()}')
        batch_size = xs_pad.size(0)
        w = dummy_w.expand(batch_size).view(-1, 1) if dummy_w is not None else None
        ys = [y[y != self.ignore_id] for y in ys_pad]
        truth = torch.stack([y[0] for y in ys])
        ...
        # logging.warning(f'{logit.size()} {w.size()}')
        if w is not None:
            logit = logit * w
        # logging.warning(f'{logit.size()} {w.size()}')
        losses = self.criterion(logit, truth)
        self.loss = losses.mean()
        penalty, penalty_data = None, None
        if w is not None and losses.requires_grad:
            penalty = self.compute_irm_penalty(losses, dummy_w)
        ...
        if self.decoder is not None:
            ...
            mask = (pred == truth)
            self.acc = float(torch.sum(mask).float() / batch_size)
        ...
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
            self.reporter.report(
                loss_ctc_data, loss_att_data, self.acc, cer_ctc, cer, wer, loss_data, penalty=penalty_data
            )
        else:
    ...
    def calculate_all_ctc_probs(self, xs_pad, ilens, ys_pad):
        ...
        with torch.no_grad():
            self.forward(xs_pad, ilens, ys_pad)
        for name, m in self.named_modules():
            # logging.warning(f'calculate_all_ctc_probs {name}')
            if isinstance(m, CTC) and name == 'ctc' and m.probs is not None:
