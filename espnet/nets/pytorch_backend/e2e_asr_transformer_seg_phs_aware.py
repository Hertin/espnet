...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def softmax_mask(self, ys_hat, lang_ids = None):
        min_value = float(numpy.finfo(torch.tensor(0, dtype=ys_hat.dtype).numpy().dtype).min)
        if lang_ids is not None:
            for batch_idx in range(len(ys_hat)):
                phs_list = []
                for lang_id in lang_ids[batch_idx]:
                    phs_list.extend(self.phs_aware_dict[int(lang_id)])
    ...
    def forward(self, langs, xs_pad, ilens, ys_pad):
        ...
        logit = self.softmax_mask(self.connected(hs_stack), lang_ids = lang_ids)
