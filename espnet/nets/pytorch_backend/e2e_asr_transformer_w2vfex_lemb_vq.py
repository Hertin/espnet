...
class Reporter(chainer.Chain):
    ...
    def report(self, loss_ctc, loss_att, acc, cer_ctc, cer, wer, mtl_loss, ppl_loss=None):
        ...
        reporter.report({"ppl_loss": ppl_loss}, self)
    ...
    def forward(self, audio_pad, audio_lens, ys_pad, lang_labels):
        ...
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
            self.reporter.report(
                loss_ctc_data, loss_att_data, self.acc, cer_ctc, cer, wer, loss_data, ppl_loss=float(self.ppl_loss)
            )
        else:
